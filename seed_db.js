...
async function seed() {
    ...
    const resLoc = await fetch(`${SUPABASE_URL}/rest/v1/locations`, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify(DEFAULT_LOCATIONS)
    });
    console.log("Locations seed status:", resLoc.status);
    
    console.log("Seeding complaints...");
    const resComp = await fetch(`${SUPABASE_URL}/rest/v1/complaints`, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify(DEFAULT_COMPLAINTS)
    });
    console.log("Complaints seed status:", resComp.status);
}
seed();
